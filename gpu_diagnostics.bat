...
powershell -NoProfile -NoLogo -Command "$g = Get-CimInstance Win32_VideoController | Where-Object { $_.Name -match 'intel|arc|xe' }; if (-not $g) { exit 1 }" >nul 2>&1
if errorlevel 1 (
  echo [WARN] No Intel Arc/Xe GPU detected via WMI/CIM
) else (
  echo [OK]  Intel GPU detected
  powershell -NoProfile -NoLogo -Command "Get-CimInstance Win32_VideoController | Where-Object { $_.Name -match 'intel' } | Select-Object Name, PnpDeviceID"
)
...
echo(
powershell -NoProfile -NoLogo -Command "Get-CimInstance Win32_VideoController | Select-Object Name, DriverVersion, VideoProcessor"
