...
ollama serve 1>> \"%LOG_DIR%\ollama-server.log\" 2>> \"%LOG_DIR%\ollama-debug.log\""
...
\"for ($i=0; $i -lt 10; $i++) { try { $r = Invoke-WebRequest -UseBasicParsing http://localhost:11434/api/tags -TimeoutSec 2; if ($r.StatusCode -eq 200) { Write-Host 'Ollama is up.'; exit 0 } } catch { } Start-Sleep -Seconds 1 } Write-Host 'Ollama did not respond in time.'; exit 1\" && ^
echo Running ollamabench... && ^
echo Output: %LOG_DIR%\benchmark-qwen35.txt && ^
python -m ollamabench.benchmark_runner %MODEL% --warmup-runs 2 > \"%LOG_DIR%\benchmark-qwen35.txt\" && ^
...
powershell -NoProfile -NoLogo -Command ^
\"Measure-Command { curl http://localhost:11434/api/generate -H 'Content-Type: application/json' -d '{\"\"model\"\": \"%MODEL%\", \"\"prompt\"\": \"\"Return OK only.\"\", \"\"stream\"\": false}' } | Out-File -Append '%LOG_DIR%\curl-latency.log'\" && ^
echo Done."
